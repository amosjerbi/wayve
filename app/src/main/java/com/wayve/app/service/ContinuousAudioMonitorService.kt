...
class ContinuousAudioMonitorService : Service() {
    ...
    private fun getShazamApiUrl(): String {
        ...
        return prefs.getString("shazam_api_url", "https://shazam-api6.p.rapidapi.com/shazam/recognize/") ?: "https://shazam-api6.p.rapidapi.com/shazam/recognize/"
    }
    ...
    private fun getShazamApiHost(): String {
        val prefs = getSharedPreferences("wayve_prefs", MODE_PRIVATE)
        return prefs.getString("shazam_api_host", "shazam-api6.p.rapidapi.com") ?: "shazam-api6.p.rapidapi.com"
    }
    
    // Audio recording settings (optimized for efficiency)
    private val SAMPLE_RATE = 8000 // Lower for battery efficiency
    private val CHANNEL_CONFIG = AudioFormat.CHANNEL_IN_MONO
    private val AUDIO_FORMAT = AudioFormat.ENCODING_PCM_16BIT
    private val RECORD_DURATION_MS = 10000 // 10 seconds per sample
    ...
    private fun recordAudio(): ByteArray? {
        ...
        try {
            ...
            audioRecord.stop()
            Log.d(TAG, "Recording complete: ${outputStream.size()} bytes")
    ...
    private suspend fun recognizeWithShazam(audioData: ByteArray): NowPlayingTrack? {
        return withContext(Dispatchers.IO) {
            try {
                ...
                // Convert PCM to WAV for Shazam API
                val wavData = convertPcmToWav(audioData)
                
                // Create multipart form request with upload_file field
    ...
    private fun parseShazamResponse(json: String): NowPlayingTrack? {
        try {
            ...
            val subtitle = track.getString("subtitle") // This is the artist
            ...
            Log.d(TAG, "✅ Detected: $subtitle - $title ${if (albumArtUrl != null) "(with album art)" else ""}")
            ...
            Log.e(TAG, "Error parsing Shazam response", e)
            return null
        }
    }
